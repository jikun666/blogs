...
1. 二进制传输更高效
    ...
    放弃原来 “header + body” 的报文结构，替换为数个小片的二进制帧。二进制相比于纯文本协议解析起来更高效。
...
2. 头部压缩
    ...
    维护一个首部表来跟踪和存储之前发送的键值对，首部表在连接存续期间内始终存在，由客户端服务端共同渐进更新。

3. 多路复用

    并行的请求能在同一个链接中处理，移除了 HTTP1.x 中顺序和阻塞的约束。

    同域名下所有通信都在单个连接上完成。单个连接可以承载任意数量的双向数据流。消除了因多个 TCP 连接而带来的延时和内存消耗。
